...
并非所有信号都需要同等严苛的规则，区分对待是解决问题的关键。

* 最高优先级：时钟（CLK）差分对：这是整个系统的时序基准，要求最严。必须保证差分阻抗100Ω、对内等长（通常要求±5mil以内，极限±30mil），并尽量满足3W间距。

* 次高优先级：DQS 差分对：DQS是数据同步信号，重要性仅次于时钟。需保证差分阻抗和严格的对内等长（如±20mil或±50mil），间距尽量满足3W。

* 高优先级：数据线（DQ）组：数据线以8位（一个字节）为一组，组内等长是关键（通常±50mil）。组内信号间的3W间距可适度放宽。

* 中等优先级：地址/控制线：这些信号速率相对较低，时序余量较大。等长要求可放宽（如允许比时钟线长1000-2500mil），3W间距也非必须。

* 简单来说，“对内等长”的优先级远高于“3W间距”。先保证等长，再尽可能优化间距。
